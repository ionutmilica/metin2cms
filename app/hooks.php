...
Event::listen('account.creating', function (&$data)
{
    ...
    {
        throw new \Metin2CMS\Exceptions\Core\RegistrationFailedException(Config::get('general.register.message'));
    }
});
...
Event::listen('account.created', function ($data)
{
    ...
    {
        $mailer = app()->make('Metin2CMS\Mailers\AccountMailer');
...
{
    $last_request = app()->make('Metin2CMS\Repositories\AccountMetaRepositoryInterface')
    ...
    if ($last_request && $until > Carbon::now()->timestamp)
    {
        ...
        throw new \Metin2CMS\Exceptions\Core\SafeboxException($message);
    }
});
...
Event::listen('account.deletion_code.before', function ($data)
{
    ...
    if ($last_request && $until > Carbon::now()->timestamp)
    {
        ...
        throw new \Metin2CMS\Exceptions\Core\DeletionCodeException($message);
    }
});
...
Event::listen('account.email.before', function ($data)
{
    ...
    if ($last_request && $until > Carbon::now()->timestamp)
    {
        ...
        throw new \Metin2CMS\Exceptions\Core\EmailFailedException($message);
    }
});